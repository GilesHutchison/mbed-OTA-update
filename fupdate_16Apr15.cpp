...
#define FU_BUFFER_SIZE  380
#define FU_TCP_TIMEOUT  30000 // milli-seconds
#define FU_PORT 80
#define FU_BLOCK 1
#define FU_NON_BLOCK 0
...
//char FUmessageBuf[FU_BUFFER_SIZE +1 ];
//extern char WSmessageBuf[];

char* RAMbuf = (char*)0x20000000;
...
TCPSocketConnection socket;
...
#endif    
    wd.Service();  
    if (socket.connect(IPaddress, FU_PORT) < 0) 
    {
#ifdef FU_DEBUG
    ...
    if (connected == 1)
    {
        ...
        wd.Service();  
        socket.set_blocking( FU_BLOCK , FU_TCP_TIMEOUT );        // Block for FU_TCP_TIMEOUT milli-seconds
#ifdef FU_DEBUG
        pc.printf("\r\nOpened Socket\r\n");
#endif  
 ...
 #ifdef FU_DEBUG
                             if ((byteCount%256) == 0){ pc.printf("byteCount=%d i=%d n=%d\r",byteCount,i,n);} 
#endif                   
                        } while (i<n);
                    }    
                    size=size+n;  
